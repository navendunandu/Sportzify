...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    {
        $selUser="select MAX(booking_id) as bid from tbl_pbook where user_id=".$_SESSION['uid'];
        $res=$conn->query($selUser);
        $data=$res->fetch_assoc();
         $up ="update tbl_pbook set booking_status='2' where booking_id='".$data["bid"]."'";
        if($conn->query($up))
        {
            ?>
            <script>
                window.location="PSuccess.html"
                </script>
            <?php
        }
    }?>
