...
<html>
    ...
    <?php
        ...
        if (isset($_POST["btn_checkout"])) {
                ...
                $selC = "select * from tbl_pbook where user_id='" .$_SESSION["uid"]. "'and booking_status='0'";
                $rs = $conn->query($selC);
                $row=$rs->fetch_assoc();
                 $_SESSION["bid"] = $row["booking_id"];
                
                $upQry1 = "update tbl_pbook set booking_date=curdate(),booking_amount='".$amt."',booking_status='1' where user_id='" .$_SESSION["uid"]. "'";
				$conn->query($upQry1);
                ...
                if($conn->query($upQry1))
                {
					...
					if(isset($_POST["cb_checkout"]))
					{
						?>
                    <script>
					window.location="ProductPayment.php";
					</script>
                    <?php
					}
					else
...
<form method="post">
        <div class="shopping-cart" style="margin-top: 50px">            <?php                
            $sel = "select * from tbl_pbook b inner join tbl_cart c on c.booking_id=b.booking_id where b.user_id='" .$_SESSION["uid"]. "' and booking_status='0' and cart_status=0";
            $res = $conn->query($sel);
            ...
            <div class="product">
                <div class="product-image">
                    <img
                        src="../Assets/Files/Product/<?php echo $rowpr["product_image"] ?>"
                        />
                </div>
                <div class="product-details">
                    <div class="product-title"><?php echo $rowpr["product_name"] ?></div>
                    <p class="product-description">
                    <?php echo $rowpr["product_detail"] ?>
                    </p>
                </div>
